...
public class SignUpServlet  extends HttpServlet {
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if(check==true)
		{
			resp.getWriter().println("아이디가 중복됩니다</br>");
			resp.getWriter().println("<a href='signup.html'>go to main</a>");
		}
		else
		{
			UserAccount ua = new UserAccount(id,password,name);
			pm.makePersistent(ua);
			resp.getWriter().println("회원가입이 되었습니다</br>");
			resp.getWriter().println("<a href='login.html'>로그인페이지로</a>");
		}
